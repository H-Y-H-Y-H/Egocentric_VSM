...
urdf_path = 'robot_repo/'

p.connect(p.GUI)
p.configureDebugVisualizer(p.COV_ENABLE_RENDERING, 0)
atlas = p.loadURDF(urdf_path+"atlas/atlas_v4_with_multisense.urdf", [-2, 3, -0.5])
for i in range(p.getNumJoints(atlas)):
    p.setJointMotorControl2(atlas, i, p.POSITION_CONTROL, 0)
    print(p.getJointInfo(atlas, i))

if 1:
    objs = p.loadSDF(urdf_path+"botlab/botlab.sdf", globalScaling=2.0)
    zero = [0, 0, 0]
...
p.loadURDF(urdf_path+"boston_box.urdf", [-2, 3, -2], useFixedBase=True)

p.resetDebugVisualizerCamera(cameraDistance=1, cameraYaw=148, cameraPitch=-9, cameraTargetPosition=[0.36, 5.3, -0.62])

p.loadURDF(urdf_path+"boston_box.urdf", [0, 3, -2], useFixedBase=True)
    ...
    for k in keys:
        if (keys[k] & p.KEY_WAS_TRIGGERED):
            if (k == ord('i')):
                x = 10. * math.sin(t)
                y = 10. * math.cos(t)
                p.getCameraImage(320, 200, lightDirection=[x, y, 10],
