...
def data_collection(env, steps, parameters, GAUSSIAN, save_path='./', noise=0.1):
    ...
    for i in range(steps):
        ...
        para_tuned = np.random.normal(loc=parameters, scale=noise, size=None)
        action = move_altas(ti, para_tuned)

        ns, r, done, _ = env.step(action)  # ns == next state
        S.append(s)
        A.append(action)
        NS.append(ns)
...
def start_train_model(env, num_data, batch_size=8, lr=1e-1, use_DataLoader=False, scale=True,
    ...
    if use_DataLoader == True:
        ...
        for i in range(train_packages, num_data):
            ...
            vA = np.loadtxt(dataset_path + "%d/A.csv" % i)
...
if __name__ == '__main__':
    ...
    if RUN_PROGRAM == 1:
        ...
        for pack_idx in start_end:
            ...
            os.makedirs(sp_frame,exist_ok=True)

            env.robot_view_path = sp_frame
            env.counting = 0
            data_collection(env, steps=int(data_num * 1000), parameters=para, GAUSSIAN=1, noise=noise, save_path=sp)
